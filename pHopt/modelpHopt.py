...
from model_cls.modelcls import Mymodel_ph
from model_reg78.modelreg import Mymodel_phreg1
from model_regrest.modelreg import Mymodel_phreg2





DEFAULT_CLS_CKPT = os.path.join(current_dir, '../best_model/pHopt/classifier/model_cls.pt')
DEFAULT_REG1_CKPT = os.path.join(current_dir, '../best_model/pHopt/Regressor_1/model_reg78.pt')
DEFAULT_REG2_CKPT = os.path.join(current_dir, '../best_model/pHopt/Regressor_2/model_12.pt')
...
class FinalModel(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.cls_model = Mymodel_ph(vocab_size=vocab_size).to(device)
        self.reg_model1 = Mymodel_phreg1(vocab_size=vocab_size).to(device)
        self.reg_model2 = Mymodel_phreg2(vocab_size=vocab_size).to(device)
        ...
        if reg1_ckpt_path and os.path.exists(reg1_ckpt_path):
            ...
            print(f"✅ Loaded Reg78 model weights from: {reg1_ckpt_path}")
        else:
            print(f"⚠️ Warning: Reg78 weights not found at {reg1_ckpt_path}")
