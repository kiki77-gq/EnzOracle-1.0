...
class PositionalEncoding_padding(nn.Module):
    def __init__(self, d_model=128, max_len=1024, dropout=0.3):
...
class CrossAttentionBlock(nn.Module):
    ...
    def forward(self, seq_features, esm_features, esm_mask):
        """
        seq_features: [B, L, hid_dim]
        esm_features: [B, L, hid_dim]
        esm_mask: [B, L] True for valid tokens
        """
...
class ExtraEncoder(nn.Module):
    ...
    def __init__(self, d_model, num_layers=2, nhead=2, dim_feedforward=512, dropout=0.3):
        super().__init__()
...
class predictModel(nn.Module):
    def __init__(self, seq_len, enc_dim=128, esm_dim=1280, proj_dim=256, conv_out=512, num_extra_encoder_layers=1):
        ...
        self.cross_attn = CrossAttentionBlock(hid_dim=proj_dim, num_heads=2, dropout=0.3)
        self.extra_encoder = ExtraEncoder(d_model=proj_dim, num_layers=num_extra_encoder_layers, nhead=2,
                                          dim_feedforward=proj_dim * 4, dropout=0.3)
...
class Mymodel_phreg1(nn.Module):
    def __init__(self, vocab_size):
        super(Mymodel_phreg1, self).__init__()
        self.use_cuda = use_cuda
