...
class CrossAttentionBlock(nn.Module):
    ...
    def forward(self, seq_features, esm_features, esm_mask):
        ...
        """
        #mask_expanded = esm_mask.unsqueeze(1)  # [B, 1, L]
...
class predictModel(nn.Module):
    ...
    def forward(self, seq_feats, esm_feats=None, esm_mask=None):
        ...
        cls_logit = self.classifier(fusion)  # [B, num_bins]
...
class Mymodel_ph(nn.Module):
    def __init__(self, vocab_size):
        super(Mymodel_ph, self).__init__()
        self.use_cuda = use_cuda
    ...
    def forward(self, seq_inputs, esm_embeddings=None, esm_mask=None):
        ...
        if esm_embeddings is not None and esm_mask is not None:
            ...
            cls_logit, fusion, att_weights = self.predictor(enc_outputs, esm_embeddings, esm_mask=esm_mask)
        else:
            cls_logit, fusion, att_weights = self.predictor(enc_outputs, None, mask=(seq_inputs != 0), esm_mask=None)
        return cls_logit, fusion, att_weights
