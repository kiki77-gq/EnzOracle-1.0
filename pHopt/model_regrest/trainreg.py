...
def train_one_epoch(model, dataloader, optimizer, device, reg_loss_func, 
                    epoch_idx=None, total_epochs=None, use_amp=True, 
    ...
    for i, batch in enumerate(dataloader):
        ...
        with torch.cuda.amp.autocast(enabled=use_amp):
            ...
            loss = reg_loss_func(pred_reg, reg_labels, weight=weight)
        ...
        total_loss_samples += loss_val * batch_size        
    ...
    real_true = all_reg_true * 11.0 + 1.0
    real_pred = all_reg_pred * 11.0 + 1.0
...
def valid_one_epoch(model, dataloader, device, reg_loss_func, use_amp=False, linear_correction=True):
    ...
    norm_true = np.concatenate(all_reg_true, axis=0)
    norm_pred = np.concatenate(all_reg_pred, axis=0)


    real_true = norm_true * 11.0 + 1.0
    real_pred = norm_pred * 11.0 + 1.0
...
def parse_args():
    ...
    parser.add_argument('--espatience', default=30, type=int)
...
def main():
    ...
    model, optimizer, scheduler, progress, paths = prepare_training_independent(
        args.save_dir, model_name="phreg12", build_model_fn=lambda: model,
