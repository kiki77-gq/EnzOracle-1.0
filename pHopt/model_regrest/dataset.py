...
class SeqDataset(Data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        reg_label = float(row['pHopt'])
        norm_label = (reg_label - 1.0) / 11.0
        norm_label = max(0.0, min(1.0, norm_label))
        ...
        return torch.LongTensor(seq_input), torch.FloatTensor([norm_label]), esm_feature, esm_mask, torch.FloatTensor([weight]), seq_id 
