...
from pHopt.modelpHopt import FinalModel



def parse_args():
    parser = argparse.ArgumentParser(description="pHopt Prediction Tool for User Sequences")
    ...
    parser.add_argument('--db_path', type=str, 
                        default=os.path.join(current_dir, 'data/pHopt/pHopt.csv'), 
...
def main():
    ...
    if os.path.exists(args.db_path):
        ...
        db_dict = dict(zip(db_df[seq_col], db_df['pHopt']))
    ...
    if len(to_predict) > 0:
        ...
        print("🔮 Loading Final pHopt Prediction Model...")
        ...
        for i in tqdm(range(0, len(to_predict), args.batch_size)):
            ...
            for b_idx, item in enumerate(batch):
                ...
                seq_input = [vocab.get(s, vocab.get('-')) for s in padded_seq] # 处理未知氨基酸
