...
def train_one_epoch(model, dataloader, optimizer, device, cls_loss_func,  
    ...
    for i, batch in enumerate(pbar):
        ...
        cls_probs = torch.sigmoid(cls_probs)
        current_loss = loss.item() * accumulation_steps 
        total_loss_samples += current_loss * batch_size
...
class LabelSmoothingBCEWithLogitsLoss(nn.Module):
    ...
    def __init__(self, smoothing=0.1, reduction='mean', pos_weight=None):
       

        super().__init__()
        assert 0 <= smoothing < 1, "Smoothing value must be in [0, 1)"
...
def parse_args():
    ...
    default_csv = os.path.join(current_dir, '../../data/Topt/Topt.csv')
    ...
    parser.add_argument('--espatience', default=12, type=int)
    parser.add_argument('--lr', default=5e-5, type=float)
    parser.add_argument('--weight_decay', default=1e-2, type=float)
    parser.add_argument('--accumulation_steps', default=4, type=int)
    parser.add_argument('--scheduler_type', default='cosine', type=str, choices=['plateau', 'cosine'], help="Type of learning rate scheduler (default: cosine)")
...
def main():
    ...
    cls_loss_func = LabelSmoothingBCEWithLogitsLoss(smoothing=0.1)


    model = Mymodel_topt(vocab_size=vocab_size).to(device)
    ...
    model, optimizer, scheduler, progress, paths = prepare_training_independent(
        args.save_dir, model_name="toptcls", build_model_fn=lambda: model,
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_loss, train_metrics = train_one_epoch(
            ...
            epoch_idx=epoch+1, total_epochs=args.epochs, use_amp=True, 
