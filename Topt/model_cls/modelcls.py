...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, Q, K, V, attn_mask):
        ...
        scores.masked_fill_(attn_mask, fill_value)
...
class AttentionPooling(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        if mask is not None:
            ...
            fill_value = -1e4 if score.dtype == torch.float16 else -1e9
            score = score.masked_fill(~mask, fill_value)
...
class predictModel(nn.Module):
    def __init__(self, seq_len, enc_dim=128, esm_dim=1280, proj_dim=128, conv_out=128, num_extra_encoder_layers=1):
        ...
        self.cross_attn = CrossAttentionBlock(hid_dim=proj_dim, num_heads=2, dropout=0.5)
        self.extra_encoder = ExtraEncoder(d_model=proj_dim, num_layers=num_extra_encoder_layers, nhead=2,
                                          dim_feedforward=proj_dim * 4, dropout=0.5)
    ...
    def forward(self, seq_feats, esm_feats=None, esm_mask=None):
        ...
        cls_logit = self.classifier(fusion)  
        

         
        return cls_logit, fusion, att_weights
...
class Mymodel_topt(nn.Module):
    def __init__(self, vocab_size):
        super(Mymodel_topt, self).__init__()
        self.use_cuda = use_cuda
        ...
        self.predictor = predictModel(seq_len=1024,
                                      ...
                                      proj_dim=128,
                                      conv_out=128,
                                      num_extra_encoder_layers=1).to(device)
    ...
    def forward(self, seq_inputs, esm_embeddings=None, esm_mask=None):
        ...
        if esm_embeddings is not None and esm_mask is not None:
            ...
            cls_probs, fusion, att_weights = self.predictor(enc_outputs, esm_embeddings, esm_mask=esm_mask)
        else:
            cls_probs, fusion, att_weights = self.predictor(enc_outputs, None, mask=(seq_inputs != 0), esm_mask=None)
        return cls_probs, fusion, att_weights
