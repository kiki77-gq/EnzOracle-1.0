...
def train_one_epoch(model, dataloader, optimizer, device, reg_loss_func, 
    ...
    for batch in pbar:
        ...
        all_reg_true.append((reg_labels * (tm_max - tm_min) + tm_min).detach().cpu().numpy())
        all_reg_pred.append((pred_reg * (tm_max - tm_min) + tm_min).detach().cpu().numpy())
...
def valid_one_epoch(model, dataloader, device, reg_loss_func, use_amp=False):
...
class RegFocalLoss(_Loss):
    ...
    def forward(self, pred_reg, target, weight=None):
       
        se = torch.abs(pred_reg - target.view(-1))  # [B]
        focal_weight = torch.pow(se, self.gamma).detach()
        focal_weight = focal_weight / (focal_weight.sum() + 1e-8)  # normalize

        loss = (se ** 2) * focal_weight
...
def parse_args():
    ...
    default_csv = os.path.join(current_dir, '../../data/Topt/topt_20_40.csv')
    default_esm = os.path.join(current_dir, '../../data/Topt/esm_embedding')
    default_save = os.path.join(current_dir, '../../train_model/Topt/savetrain')
    ...
    parser.add_argument('--scheduler_type', default='plateau', type=str, choices=['plateau', 'cosine'], help="Type of learning rate scheduler (default: cosine)")
...
def main():
    ...
    model = Mymodel_toptreg1(vocab_size=vocab_size).to(device)
    ...
    model, optimizer, scheduler, progress, paths = prepare_training_independent(
        args.save_dir, model_name="toptreg2040", build_model_fn=lambda: model,
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if scheduler is not None:
            scheduler.step(valid_metrics['R2'])
        ...
        epoch_save_path = os.path.join(paths['savepath'], f"model_epoch_{epoch}.pt")
        torch.save(checkpoint_state, epoch_save_path)
        print(f"💾 Saved checkpoint: {epoch_save_path}")

        
        torch.save(checkpoint_state, paths['recent_model'])
