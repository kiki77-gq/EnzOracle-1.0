...
def parse_args():
    ...
    default_csv = os.path.join(current_dir, '../../data/Tm/tm_rest.csv')
...
def main():
    ...
    model = Mymodel_tmreg2(vocab_size=vocab_size).to(device)
    ...
    model, optimizer, scheduler, progress, paths = prepare_training_independent(
        args.save_dir, model_name="tmreg100", build_model_fn=lambda: model,
