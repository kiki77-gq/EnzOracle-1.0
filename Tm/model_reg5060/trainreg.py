...
def train_one_epoch(model, dataloader, optimizer, device, reg_loss_func, 
                    epoch_idx=None, total_epochs=None, use_amp=False, scheduler=None):
    ...
    for batch in pbar:
        ...
        with torch.cuda.amp.autocast(enabled=use_amp):
            ...
            pred_reg, fusion, weights = model(seq_inputs, esm_embeddings, esm_mask)  
...
def parse_args():
    ...
    default_csv = os.path.join(current_dir, '../../data/Tm/tm_50_60.csv')
    ...
    parser.add_argument('--espatience', default=20, type=int)
...
def main():
    ...
    reg_loss_func = WeightedSmoothL1Loss(beta=0.1, reduction='mean')

    model = Mymodel_tmreg1(vocab_size=vocab_size).to(device)
    ...
    def scheduler_fn(optimizer):
        ...
        scheduler = None
        if args.scheduler_type.lower() == "plateau":
            scheduler = optim.lr_scheduler.ReduceLROnPlateau(
    ...
    model, optimizer, scheduler, progress, paths = prepare_training_independent(
        args.save_dir, model_name="tmreg5060", build_model_fn=lambda: model,
