...
class SeqDataset(Data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #seq = row['Sequence'].ljust(self.seq_max_len, '-')
        seq_input = [self.vocab[n] for n in seq]

        #reg_label = (row['tm'] - 50.0) / (100.0 - 50.0)
        reg_label = row['tm']
        class_label = float(row['in_50_60'])
...
def cls_data_load_seq(csv_path, batch_size, split='Training',esm_loader=None, seq_max_len=1024):
